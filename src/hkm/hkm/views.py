...
class BaseView(TemplateView):
	...
	def handle_invalid_post_action(self, request, *args, **kwargs):
		LOG.error('Invalid POST action', extra={'data': {'POST': repr(request.POST), 'permissions': self.permissions}})
		return http.HttpResponseBadRequest()
...
class CollectionDetailView(BaseView):
	...
	def get_context_data(self, **kwargs):
		...
		if self.collection_record:
			...
			context['record_web_url'] = FINNA.get_image_url(self.collection_record.record_id)
			...
			if finnaRecord:
				...
				if self.request.user.is_authenticated():
					try:
						favorites_collection = Collection.objects.get(owner=self.request.user, collection_type=Collection.TYPE_FAVORITE)
					except Collection.DoesNotExist:
						pass
					else:
						context['is_favorite'] = favorites_collection.records.filter(record_id=context['record']['id']).exists()				
...
class FinnaRecordDetailView(BaseFinnaRecordDetailView):
	...
	def get_context_data(self, **kwargs):
		...
		if not self.record:
			return context
		related_collections_ids = Record.objects.filter(record_id=self.record['id']).values_list('collection', flat=True) # WAT... no objects in Record model
		related_collections = Collection.objects.user_can_view(self.request.user).filter(id__in=related_collections_ids).distinct()
		context['related_collections'] = related_collections
		return context
...
class OrderSummaryView(BaseOrderView):
	...
	def post(self, request, *args, **kwargs):
		...
		if action == 'order-submit':
			...
			LOG.debug('ORDER ATTEMPT STARTED AT: ', extra={'data': {'order_hash': self.order.order_hash, 'time': str(self.order.datetime_checkout_started)}})
			self.order.save()
...
# If and only if the request parameter 'settled' is true, the order will be sent to print
...
# In testing API there is never time lag with this, but in real system there can be
