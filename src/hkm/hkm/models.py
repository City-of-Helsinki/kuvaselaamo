...
class Record(OrderedModel, BaseModel):
	...
	def get_preview_image_absolute_url(self):
		LOG.debug('Getting web image absolute url', extra={'data': {'finna_id': self.record_id}})
...
class ProductOrder(BaseModel):
	...
	def handle_confirmation(self, result):
		...
		if not self.datetime_checkout_ended:
			...
			LOG.debug('CHECKOUT ENDED AT: ', extra={'data': {'order_hash': self.order_hash, 'time': str(self.datetime_checkout_ended)}})
		
		# if return code for checkout is 0 (SUCCESS) 
		if result['return_code'] == '0':			
			# if order not yet marked as successful,
			# send order confirmation mail to customer and mark checkout as successful
			if not self.is_checkout_successful:
				LOG.debug('CHECKOUT SUCCESSFUL ', extra={'data': {'order_hash': self.order_hash}})
			...
			if result['settled'] == '1':
				...
				# Mark it as such and try to send order details to Printing Agency
				if not self.is_payment_successful:
					self.is_payment_successful = True
					LOG.debug('PAYMENT SETTLED SUCCESSFULLY ', extra={'data': {'order_hash': self.order_hash}})

				# If order has not yet been marked as successfully sent, send it
				# PRINTMOTOR post method returns status code of Send Print Order Request
				if not self.is_order_successful:
					LOG.debug('sending to Printmotor')
					self.datetime_order_started = datetime.datetime.now()
					LOG.debug('PRINT ORDER ATTEMPT STARTED AT: ', extra={'data': {'order_hash': self.order_hash, 'time': str(self.datetime_order_started)}})
					printOrder = PRINTMOTOR.post(self)
					self.datetime_order_ended = datetime.datetime.now()
									
					if printOrder:
						LOG.debug('PRINT ORDER ATTEMPT ENDED AT: ', extra={'data': {'order_hash': self.order_hash, 'time': str(self.datetime_order_ended)}})
						if printOrder == 200:
