...
from hkm.management.commands.clean_unused_data import (
    DEFAULT_DAYS_UNTIL_NOTIFICATION,
    DEFAULT_DAYS_UNTIL_REMOVAL,
)
...
def test_that_old_anonymous_data_is_removed_using_default_date(
...
@pytest.mark.django_db
def test_that_old_anonymous_data_is_removed_using_custom_date(
    day_older_than_custom_removal_date, day_newer_than_custom_removal_date
):
    ...
    call_command(
        "clean_unused_data",
        days_until_removal=CUSTOM_DAYS_UNTIL_REMOVAL,
...
@pytest.mark.django_db
def test_that_old_user_gets_deleted_using_default_date(
    day_older_than_removal_date,
    day_newer_than_removal_date,
    day_outside_grace_period,
    day_within_grace_period,
):
    not_deleted = [
        UserFactory(
            last_login=day_newer_than_removal_date,
            profile__removal_notification_sent=None,
        ),
        UserFactory(
            last_login=day_newer_than_removal_date,
            profile__removal_notification_sent=day_within_grace_period,
        ),
    ]

    UserFactory(
        last_login=day_older_than_removal_date,
        profile__removal_notification_sent=day_outside_grace_period,
    )
...
@pytest.mark.django_db
def test_that_old_user_gets_deleted_using_custom_date(
    day_older_than_custom_removal_date,
    ...
    UserFactory(
        last_login=day_older_than_custom_removal_date,
...
@pytest.mark.django_db
def test_that_user_is_not_deleted_without_notification(
    day_older_than_removal_date, day_within_grace_period, day_outside_grace_period
):
    not_deleted = [
        UserFactory(
            last_login=day_older_than_removal_date,
